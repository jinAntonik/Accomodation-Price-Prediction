...
**Данные:** объявления с сайта Циан о продаже квартир в
Санкт-Петербурге (вторичный рынок жилья)

**"Личный риэлтор"** -- сервис, формирующий цену, по которой
пользователь может выгодно продать свою квартиру, не прибегая к услугам
агента по продаже жилья. На вход принимает характеристики квартиры, а
выдает предсказанную стоимость и карту с квартирами, имеющими похожие
параметры. Таким образом, пользователь экономит на оценщике.

*План работы:*

1.  Парсинг данных с Циан при помощи кода Python
2.  Очистка данных и уточнение состава переменных
3.  Получение координат квартир (посредством кода R и вручную)
4.  Создание content-based рекомендательной системы
5.  Создание предсказательной модели при помощи ML
6.  Создание интерфейса на shiny
7.  Интеграция рекомендательной системы и предсказательной модели в
    приложение

На данном этапе приложение универсально и не предусматривает сценарии
для разных типов пользователей. Однако в дальнейшем возможно добавление
секции "Быстрый поиск" (принимает на ввод 4-5 основных характеристик и
быстрее выдает результат) и "Расширенный поиск" (необходимо ввести все
14 характеристик, и несмотря на более долгое время ожидания, получить
более точное предсказание).

## Распределение ролей

1.  **Алексеев Антон:** сбор и обработка данных, построение ML модели
2.  **Винниченко Валерия:** обработка данных, разработка интерфейса
3.  **Винниченко Вероника:** визуализация результатов, разработка
    интерфейса
4.  **Дьячкова Алиса:** планирование исследования, разработка интерфейса
5.  **Жучков Дмитрий:** сбор данных, построение рекомендательной системы

**Все члены команды:** парсинг данных, разработка сценариев
тестирования, создание дневников

## Предобработка

Первым шагом стал парсинг данных с Циан, базы объявлений о продаже
недвижимости, с помощью кода на Python по критериям. Было получено около
18100 наблюдений по 6-ти характеристикам (адрес, цена, ближайшее метро и
расстояние до него, общая информация о квартире, информация о доме и
описание).

С помощью регулярных выражений из общей информации о квартире были
извлечены необходимые переменные. Некоторые переменные, такие как высота
потолка и наличие паркинга, были удалены из-за большого количества
пропусков по ним. Также около трети наблюдений было удалено из-за
существенно неполного заполнения объявлений при их размещении (много
пропусков внутри объявления). В случае, если переменная была ключевой
для определения цены (например, наличие лифта), а пропусков оставалось
много, NA заменялись на 0. В результате, осталось 5807 наблюдений по уже
15-ти характеристикам (количество комнат, общая площадь, жилая площадь,
площадь кухни, тип ремонта квартиры, наличие балкона, вид санузла,
количество санузлов, этаж, год постройки дома, тип дома, наличие лифта,
название района, ближайшее метро, минут до метро пешком).

Таким образом, рекомендательную систему мы базировали именно на
полученных 15-ти характеристиках.

## Методы машинного обучения

С помощью машинного обучения происходит выполнение главной цели сервиса
-- предсказание цены продажи квартиры на основе ее параметров. Нами
применялось обучение с "учителем", чтобы обучить такие алгоритмы ML, как
линейная регрессия, RIDGE-регрессия, LASSO-регрессия, случайный лес,
градиентный бустинг. Алгоритмы оформлены в виде ансамбля, что и
представляет собой конечную модель. Ансамбль имеет простейшую структуру:
итоговое предсказание является суммой взвешенных предсказания отдельно
работающих моделей. В ходе интеграции предсказательной модели в
интерфейс, была выявлена ошибка в работе модели градиентного бустинга,
из-за чего от нее в итоге пришлось отказаться.

## Архитектура приложения

Для каждого параметра была создана отдельная плашка и установлены
дефолтные значения для более точного предсказания и дополнительной
мотивации пользователя предоставить все свои данные. Предобработка в
приложении не предусмотрена, используются финальные данные. Из
параметров, введенных пользователем, формируется датасет, который в
последствии прикрепляется к рекомендательной системе. Данные не
сохраняются. Уже обученные на обучающей выборке модели загружаются в
интерфейс, что позволяет значительно ускорить появление результата путем
только лишь предсказания на данных пользователя.
...
-   Карта с рекомендацией квартир со схожими характеристиками
...
Оценивание было преимущественно пользовательское: мы разослали ссылку на
приложение потенциальным пользователям (однокурсникам) с целью получения
фидбэка и экспертное (самооценивание).

В результате пользовательского оценивания, большинство было
удовлетворено работой и интерфейсом приложения, а также точностью
предсказаний. Дополнительно пользователи предлагали добавить опцию
"неважно" для каждой из характеристик, сделать опции быстрого (4-5
основных характеристик) и расширенного поиска (все 15 характеристик),
исправить некоторые неточности визуализации (иногда цена отображалась в
научном стиле, т.е. 8е+06) и настроить выдачу ошибки для несовместимых
запросов (например, жилая площадь больше общей площади).

Дальнейшие шаги по развитию приложения:

-   Создание автообновляемой базы объявлений
-   Предупреждение о вводе недопустимых значений
-   Включение идеи связанных списков (например, нельзя после ввода
    района выбрать станцию метро, явно более дальнюю, чем другие)
-   Сохранение введенных значений при выходе
-   Увеличение скорости работы сервиса
